...
export var ToolbarButtonRenderer = function ToolbarButtonRenderer(_ref2) {
  ...
  if (href !== undefined) {
    if (href.indexOf('#!/') < 0) { // 重置单个例子利用返回组件列表按钮的href
      const { hash, pathname } = location
      const arr = hash.split('/')
      const lastIndex = Number(arr[arr.length - 1]) ? arr.length - 2 : arr.length - 1
      const path = arr.slice(1, lastIndex)
      href = pathname + '#/' + path.join('/')
    }
